import 'babel-polyfill';
import Koa from 'koa';
import Router from 'koa-router';
import mysql from 'mysql2/promise';
import KoaBody from 'koa-bodyparser';
import Url from 'url';

import { connectionSettings } from './settings';
import { databaseReady } from './helpers/loansystem';
import { initDB } from './fixtures/loansystem';


// Initialize DB
(async () => {
  await databaseReady();
  await initDB();
})();

// The port that this server will run on, defaults to 9000
const port = process.env.PORT || 9000;

// Instantiate a Koa server
const app = new Koa();
const koaBody = new KoaBody();

// Instantiate routers
const test = new Router();
const todos = new Router();

const equipments = new Router();
const users = new Router();
const loans = new Router();

// Define API path
const apiPath = '/api/v1';

test.get(`${apiPath}/test`, async (ctx) => {
  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
        SELECT *
        FROM test_table
      `);

    console.log('Data fetched:', data);

    // Tell the HTTP response that it contains JSON data encoded in UTF-8
    ctx.type = 'application/json; charset=utf-8';

    // Add stuff to response body
    ctx.body = { greeting: 'Hello world from test!', data };
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// Middleware for checking accept headers
const checkAccept = async (ctx, next) => {
  console.log('checkAccept');
  // If client does not accept 'application/json' as response type, throw '406 Not Acceptable'
  if (!ctx.accepts('application/json')) {
    ctx.throw(406);
  }
  // Set the response content type
  ctx.type = 'application/json; charset=utf-8';
  // Move to next middleware
  await next();
};

// Middleware for checking request body content
const checkContent = async (ctx, next) => {
  console.log('checkContent');
  // Check that the request content type is 'application/json'
  if (!ctx.is('application/json')) {
    ctx.throw(415, 'Request must be application/json');
  }
  // Move to next middleware
  await next();
};

// TODOS
// Define todos paths
const todosPath = `${apiPath}/todos`;
const todoPath = `${todosPath}/:id`;

// GET /resource
todos.get(todosPath, checkAccept, async (ctx) => {
  const url = Url.parse(ctx.url, true);
  const { sort } = url.query;

  const parseSortQuery = ({ urlSortQuery, whitelist }) => {
    let query = '';
    if (urlSortQuery) {
      const sortParams = urlSortQuery.split(',');

      query = 'ORDER BY ';
      sortParams.forEach((param, index) => {
        let trimmedParam = param;
        let desc = false;

        if (param[0] === '-') {
          // Remove the first character
          trimmedParam = param.slice(1);
          // Set descending to true
          desc = true;
        }

        // If parameter is not whitelisted, ignore it
        // This also prevents SQL injection even without statement preparation
        if (!whitelist.includes(trimmedParam)) return;

        // If this is not the first sort parameter, append ', '
        if (index > 0) query = query.concat(', ');

        // Append the name of the field
        query = query.concat(trimmedParam);

        if (desc) query = query.concat(' DESC');
      });
    }
    return query;
  };
  const orderBy = parseSortQuery({ urlSortQuery: sort, whitelist: ['id', 'text', 'done'] });

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
        SELECT *
        FROM todos
        ${orderBy}
      `);

    // Return all todos
    ctx.body = data;
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// GET /resource/:id
todos.get(todoPath, checkAccept, async (ctx) => {
  const { id } = ctx.params;
  console.log('.get id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
          SELECT *
          FROM todos
          WHERE id = :id;
        `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// POST /resource
todos.post(todosPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { text } = ctx.request.body;
  console.log('.post text contains:', text);

  if (typeof text === 'undefined') {
    ctx.throw(400, 'body.text is required');
  } else if (typeof text !== 'string') {
    ctx.throw(400, 'body.done must be string');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Insert a new todo
    const [status] = await conn.execute(`
         INSERT INTO todos (text)
         VALUES (:text);
       `, { text });
    const { insertId } = status;

    // Get the new todo
    const [data] = await conn.execute(`
         SELECT *
         FROM todos
         WHERE id = :id;
       `, { id: insertId });

    // Set the response header to 201 Created
    ctx.status = 201;

    // Set the Location header to point to the new resource
    const newUrl = `${ctx.host}${Router.url(todoPath, { id: insertId })}`;
    ctx.set('Location', newUrl);

    // Return the new todo
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// PUT /resource/:id
todos.put(todoPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { id } = ctx.params;
  const { text, done } = ctx.request.body;
  console.log('.put id contains:', id);
  console.log('.put text contains:', text);
  console.log('.put done contains:', done);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  } else if (typeof text === 'undefined') {
    ctx.throw(400, 'body.text is required');
  } else if (typeof text !== 'string') {
    ctx.throw(400, 'body.done must be string');
  } else if (typeof done === 'undefined') {
    ctx.throw(400, 'body.done is required');
  } else if (typeof done !== 'boolean') {
    ctx.throw(400, 'body.done must be boolean');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Update the todo
    const [status] = await conn.execute(`
           UPDATE todos
           SET text = :text, done = :done
           WHERE id = :id;
         `, { id, text, done: Number(done) });

    if (status.affectedRows === 0) {
      // If the resource does not already exist, create it
      await conn.execute(`
          INSERT INTO todos (id, text, done)
          VALUES (:id, :text, :done);
        `, { id, text, done: Number(done) });
    }

    // Get the todo
    const [data] = await conn.execute(`
           SELECT *
           FROM todos
           WHERE id = :id;
         `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// DELETE /resource/:id
todos.del(todoPath, async (ctx) => {
  const { id } = ctx.params;
  console.log('.del id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [status] = await conn.execute(`
          DELETE FROM todos
          WHERE id = :id;
        `, { id });

    if (status.affectedRows === 0) {
      // The row did not exist, return '404 Not found'
      ctx.status = 404;
    } else {
      // Return '204 No Content' status code for successful delete
      ctx.status = 204;
    }
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// EQUIPMENTS
// Define equipments paths
const equipmentsPath = `${apiPath}/equipments`;
const equipmentPath = `${equipmentsPath}/:id`;

// GET /resource
equipments.get(equipmentsPath, checkAccept, async (ctx) => {
  const url = Url.parse(ctx.url, true);
  const { sort } = url.query;

  const parseSortQuery = ({ urlSortQuery, whitelist }) => {
    let query = '';
    if (urlSortQuery) {
      const sortParams = urlSortQuery.split(',');

      query = 'ORDER BY ';
      sortParams.forEach((param, index) => {
        let trimmedParam = param;
        let desc = false;

        if (param[0] === '-') {
          // Remove the first character
          trimmedParam = param.slice(1);
          // Set descending to true
          desc = true;
        }

        // If parameter is not whitelisted, ignore it
        // This also prevents SQL injection even without statement preparation
        if (!whitelist.includes(trimmedParam)) return;

        // If this is not the first sort parameter, append ', '
        if (index > 0) query = query.concat(', ');

        // Append the name of the field
        query = query.concat(trimmedParam);

        if (desc) query = query.concat(' DESC');
      });
    }
    return query;
  };
  const orderBy = parseSortQuery({ urlSortQuery: sort, whitelist: ['id', 'name', 'description'] });

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
        SELECT *
        FROM equipments
        ${orderBy}
      `);

    // Return all equipments
    ctx.body = data;
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// GET /resource/:id
equipments.get(equipmentPath, checkAccept, async (ctx) => {
  const { id } = ctx.params;
  console.log('.get id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
          SELECT *
          FROM equipments
          WHERE id = :id;
        `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// POST /resource
equipments.post(equipmentsPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { name } = ctx.request.body;
  console.log('.post name contains:', name);
  const { description } = ctx.request.body;
  console.log('.post description contains:', description);

  if (typeof name === 'undefined') {
    ctx.throw(400, 'body.name is required');
  } else if (typeof name !== 'string') {
    ctx.throw(400, 'body.name must be string');
  } else if (typeof description === 'undefined') {
    ctx.throw(400, 'body.description is required');
  } else if (typeof description !== 'string') {
    ctx.throw(400, 'body.description must be string');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Insert a new equipment
    const [status] = await conn.execute(`
         INSERT INTO equipments (name, description)
         VALUES (:name, :description);
       `, { name, description });
    const { insertId } = status;

    // Get the new equipment
    const [data] = await conn.execute(`
         SELECT *
         FROM equipments
         WHERE id = :id;
       `, { id: insertId });

    // Set the response header to 201 Created
    ctx.status = 201;

    // Set the Location header to point to the new resource
    const newUrl = `${ctx.host}${Router.url(equipmentPath, { id: insertId })}`;
    ctx.set('Location', newUrl);

    // Return the new todo
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// PUT /resource/:id
equipments.put(equipmentPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { id } = ctx.params;
  const { name, description } = ctx.request.body;
  console.log('.put id contains:', id);
  console.log('.put name contains:', name);
  console.log('.put description contains:', description);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  } else if (typeof name === 'undefined') {
    ctx.throw(400, 'body.name is required');
  } else if (typeof description === 'undefined') {
    ctx.throw(400, 'body.description is required');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Update the equipment
    const [status] = await conn.execute(`
           UPDATE equipments
           SET name = :name, description = :description
           WHERE id = :id;
         `, { id, name, description });

    if (status.affectedRows === 0) {
      // If the resource does not already exist, create it
      await conn.execute(`
          INSERT INTO equipments (id, name, description)
          VALUES (:id, :name, :description);
        `, { id, name, description });
    }

    // Get the equipment
    const [data] = await conn.execute(`
           SELECT *
           FROM equipments
           WHERE id = :id;
         `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// DELETE /resource/:id
equipments.del(equipmentPath, async (ctx) => {
  const { id } = ctx.params;
  console.log('.del id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [status] = await conn.execute(`
          DELETE FROM equipments
          WHERE id = :id;
        `, { id });

    if (status.affectedRows === 0) {
      // The row did not exist, return '404 Not found'
      ctx.status = 404;
    } else {
      // Return '204 No Content' status code for successful delete
      ctx.status = 204;
    }
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// USERS
// Define users paths
const usersPath = `${apiPath}/users`;
const userPath = `${usersPath}/:id`;

// GET /resource
users.get(usersPath, checkAccept, async (ctx) => {
  const url = Url.parse(ctx.url, true);
  const { sort } = url.query;

  const parseSortQuery = ({ urlSortQuery, whitelist }) => {
    let query = '';
    if (urlSortQuery) {
      const sortParams = urlSortQuery.split(',');

      query = 'ORDER BY ';
      sortParams.forEach((param, index) => {
        let trimmedParam = param;
        let desc = false;

        if (param[0] === '-') {
          // Remove the first character
          trimmedParam = param.slice(1);
          // Set descending to true
          desc = true;
        }

        // If parameter is not whitelisted, ignore it
        // This also prevents SQL injection even without statement preparation
        if (!whitelist.includes(trimmedParam)) return;

        // If this is not the first sort parameter, append ', '
        if (index > 0) query = query.concat(', ');

        // Append the name of the field
        query = query.concat(trimmedParam);

        if (desc) query = query.concat(' DESC');
      });
    }
    return query;
  };
  const orderBy = parseSortQuery({ urlSortQuery: sort, whitelist: ['id', 'name', 'email'] });

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
        SELECT *
        FROM users
        ${orderBy}
      `);

    // Return all users
    ctx.body = data;
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// GET /resource/:id
users.get(userPath, checkAccept, async (ctx) => {
  const { id } = ctx.params;
  console.log('.get id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
          SELECT *
          FROM users
          WHERE id = :id;
        `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// POST /resource
users.post(usersPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { name, email } = ctx.request.body;
  console.log('.post name contains:', name);
  console.log('.post email contains:', email);

  if (typeof name === 'undefined') {
    ctx.throw(400, 'body.name is required');
  } else if (typeof name !== 'string') {
    ctx.throw(400, 'body.name must be string');
  } else if (typeof email === 'undefined') {
    ctx.throw(400, 'body.email is required');
  } else if (typeof email !== 'string') {
    ctx.throw(400, 'body.email must be string');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Insert a new equipment
    const [status] = await conn.execute(`
         INSERT INTO users (name, email)
         VALUES (:name, :email);
       `, { name, email });
    const { insertId } = status;

    // Get the new equipment
    const [data] = await conn.execute(`
         SELECT *
         FROM users
         WHERE id = :id;
       `, { id: insertId });

    // Set the response header to 201 Created
    ctx.status = 201;

    // Set the Location header to point to the new resource
    const newUrl = `${ctx.host}${Router.url(userPath, { id: insertId })}`;
    ctx.set('Location', newUrl);

    // Return the new todo
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// PUT /resource/:id
users.put(userPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { id } = ctx.params;
  const { name, email } = ctx.request.body;
  console.log('.put id contains:', id);
  console.log('.put name contains:', name);
  console.log('.put email contains:', email);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  } else if (typeof name === 'undefined') {
    ctx.throw(400, 'body.name is required');
  } else if (typeof email === 'undefined') {
    ctx.throw(400, 'body.email is required');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Update the user
    const [status] = await conn.execute(`
           UPDATE users
           SET name = :name, email = :email
           WHERE id = :id;
         `, { id, name, email });

    if (status.affectedRows === 0) {
      // If the resource does not already exist, create it
      await conn.execute(`
          INSERT INTO users (id, name, email)
          VALUES (:id, :name, :email);
        `, { id, name, email });
    }

    // Get the user
    const [data] = await conn.execute(`
           SELECT *
           FROM users
           WHERE id = :id;
         `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// DELETE /resource/:id
users.del(userPath, async (ctx) => {
  const { id } = ctx.params;
  console.log('.del id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [status] = await conn.execute(`
          DELETE FROM users
          WHERE id = :id;
        `, { id });

    if (status.affectedRows === 0) {
      // The row did not exist, return '404 Not found'
      ctx.status = 404;
    } else {
      // Return '204 No Content' status code for successful delete
      ctx.status = 204;
    }
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// LOANS
// Define loans paths
const loansPath = `${apiPath}/loans`;
const loanPath = `${loansPath}/:id`;

// GET /resource
loans.get(loansPath, checkAccept, async (ctx) => {
  const url = Url.parse(ctx.url, true);
  const { sort } = url.query;

  const parseSortQuery = ({ urlSortQuery, whitelist }) => {
    let query = '';
    if (urlSortQuery) {
      const sortParams = urlSortQuery.split(',');

      query = 'ORDER BY ';
      sortParams.forEach((param, index) => {
        let trimmedParam = param;
        let desc = false;

        if (param[0] === '-') {
          // Remove the first character
          trimmedParam = param.slice(1);
          // Set descending to true
          desc = true;
        }

        // If parameter is not whitelisted, ignore it
        // This also prevents SQL injection even without statement preparation
        if (!whitelist.includes(trimmedParam)) return;

        // If this is not the first sort parameter, append ', '
        if (index > 0) query = query.concat(', ');

        // Append the name of the field
        query = query.concat(trimmedParam);

        if (desc) query = query.concat(' DESC');
      });
    }
    return query;
  };
  const orderBy = parseSortQuery({ urlSortQuery: sort, whitelist: ['id', 'equipmentId', 'userId', 'begins', 'ends', 'returned'] });

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
        SELECT *
        FROM loans
        ${orderBy}
      `);

    // Return all loans
    ctx.body = data;
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// GET /resource/:id
loans.get(loanPath, checkAccept, async (ctx) => {
  const { id } = ctx.params;
  console.log('.get id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [data] = await conn.execute(`
          SELECT *
          FROM loans
          WHERE id = :id;
        `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// POST /resource
loans.post(loansPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const {
    equipmentId, userId, begins, ends, returned,
  } = ctx.request.body;
  console.log('.post equipmentId contains:', equipmentId);
  console.log('.post userId contains:', userId);
  console.log('.post begins contains:', begins);
  console.log('.post ends contains:', ends);
  console.log('.post returned contains:', returned);

  if (typeof equipmentId === 'undefined') {
    ctx.throw(400, 'body.equipmentId is required');
  } else if (isNaN(equipmentId) || equipmentId.includes('.')) {
    ctx.throw(400, 'body.equipmentId must be integer');
  } else if (typeof userId === 'undefined') {
    ctx.throw(400, 'body.userId is required');
  } else if (isNaN(userId) || userId.includes('.')) {
    ctx.throw(400, 'body.userId must be integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Insert a new loan
    const [status] = await conn.execute(`
         INSERT INTO loans (equipmentId, userId, begins, ends, returned)
         VALUES (:equipmentId, :userId, :begins, :ends, :returned);
       `, {
      equipmentId, userId, begins, ends, returned,
    });
    const { insertId } = status;

    // Get the new loan
    const [data] = await conn.execute(`
         SELECT *
         FROM loans
         WHERE id = :id;
       `, { id: insertId });

    // Set the response header to 201 Created
    ctx.status = 201;

    // Set the Location header to point to the new resource
    const newUrl = `${ctx.host}${Router.url(loanPath, { id: insertId })}`;
    ctx.set('Location', newUrl);

    // Return the new loan
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// PUT /resource/:id
loans.put(loanPath, checkAccept, checkContent, koaBody, async (ctx) => {
  const { id } = ctx.params;
  const {
    equipmentId, userId, begins, ends, returned,
  } = ctx.request.body;
  console.log('.put id contains:', id);
  console.log('.put equipmentId contains:', equipmentId);
  console.log('.put userId contains:', userId);
  console.log('.put begins contains:', begins);
  console.log('.put ends contains:', ends);
  console.log('.put returned contains:', returned);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  } else if (typeof equipmentId === 'undefined') {
    ctx.throw(400, 'body.equipmentId is required');
  } else if (isNaN(equipmentId) || equipmentId.includes('.')) {
    ctx.throw(400, 'equipmentId must be an integer');
  } else if (typeof userId === 'undefined') {
    ctx.throw(400, 'body.userId is required');
  } else if (isNaN(userId) || userId.includes('.')) {
    ctx.throw(400, 'userId must be an integer');
  } else if (typeof begins === 'undefined') {
    ctx.throw(400, 'body.begins is required');
  } else if (typeof ends === 'undefined') {
    ctx.throw(400, 'body.ends is required');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);

    // Update the loan
    const [status] = await conn.execute(`
           UPDATE loans
           SET equipmentId = :equipmentId,
           userId = :userId,
           begins = :begins,
           ends = :ends,
           returned = :returned
           WHERE id = :id;
         `, {
      id, equipmentId, userId, begins, ends, returned,
    });

    if (status.affectedRows === 0) {
      // If the resource does not already exist, create it
      await conn.execute(`
          INSERT INTO loans (id, equipmentId, userId, begins, ends, returned)
          VALUES (:id, :equipmentId, :userId, :begins, :ends, :returned);
        `, {
        id, equipmentId, userId, begins, ends, returned,
      });
    }

    // Get the loan
    const [data] = await conn.execute(`
           SELECT *
           FROM loans
           WHERE id = :id;
         `, { id });

    // Return the resource
    ctx.body = data[0];
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

// DELETE /resource/:id
loans.del(loanPath, async (ctx) => {
  const { id } = ctx.params;
  console.log('.del id contains:', id);

  if (isNaN(id) || id.includes('.')) {
    ctx.throw(400, 'id must be an integer');
  }

  try {
    const conn = await mysql.createConnection(connectionSettings);
    const [status] = await conn.execute(`
          DELETE FROM loans
          WHERE id = :id;
        `, { id });

    if (status.affectedRows === 0) {
      // The row did not exist, return '404 Not found'
      ctx.status = 404;
    } else {
      // Return '204 No Content' status code for successful delete
      ctx.status = 204;
    }
  } catch (error) {
    console.error('Error occurred:', error);
    ctx.throw(500, error);
  }
});

app.use(test.routes());
app.use(test.allowedMethods());
app.use(todos.routes());
app.use(todos.allowedMethods());

app.use(equipments.routes());
app.use(equipments.allowedMethods());
app.use(users.routes());
app.use(users.allowedMethods());
app.use(loans.routes());
app.use(loans.allowedMethods());

// Start the server and keep listening on port until stopped
app.listen(port);

console.log(`App listening on port ${port}`);
